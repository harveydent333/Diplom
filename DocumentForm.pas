...
begin
    if roleUser='teacher' then
      begin
        default:='SELECT * FROM Журнал WHERE 1=1';
    ...
    if roleUser='stydent' then
      begin
        default:='SELECT * FROM Журнал WHERE Фамилия='+#39+familyUser+#39+' AND Имя='+#39+nameUser+#39+' AND Отчество='+#39+secondNameUser+#39;
        if CheckBox1.Checked = true then
          default:=default+' AND НазваниеТемы='+#39+ComboBox1.Text+#39;
