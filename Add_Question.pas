...
{
  Процедура обработки изменений в ComboBox
  Изменениет тип вопроса
}
procedure TAdd_Questions.ComboBox2Change(Sender: TObject);
begin
    varianti:='';
    fillingAnswerChoices; // Вызов процедуры заполнения вариантов ответов в переменную
    ...
    if ComboBox1.ItemIndex=1 then
       config.execRequestSQL('UPDATE Вопросы'+
                               ' SET КоличествоОтветов='+IntToStr(countQuestion)+','+
                                   ' СодержаниеВопроса='+#39+memoText+#39+','+
                                   ' ВариантыОтветов='+#39+varianti+#39+','+
                                   ' ВерныйОтвет='+#39+''+#39+', '+
                                   ' КодТипа=1'+
    ...
    defaultSettings; // Вызов процедуры применения стандартных настроек для компонентов
