...
type
  ...
  private
    ...
    procedure formCustomizationProcess(value:boolean);
  public
    { Public declarations }
  end;
...
{
  Процедура обработки нажатия кнопки "Журнал оценок"
  Формирует настройки формы "Журнал оценок", для роли пользователя
}
...
begin
    ...
    if roleUser='stydent' then
        begin
            formCustomizationProcess(false); // Вызов процедуры настройки формы
            config.selectRequestSQL(getShyrnalData+' WHERE Фамилия='+
              #39+familyUser+#39+
              ' AND Имя='+#39+nameUser+#39+
              ' AND Отчество='+#39+secondNameUser+#39
            );
        end;

    if roleUser='teacher' then
        begin
            formCustomizationProcess(true); // Вызов процедуры настройки формы
            config.selectRequestSQL(getShyrnalData+' WHERE ЖурналОценок.КодУчителя='+IntToStr(kodUser));
        end;
end;

procedure TMainMenu.formCustomizationProcess(value:boolean);
begin
    settingForShyrnal(value);  // Вызов процедуры настройки компонентов формы "Журнал оценок"
    getDataTemaForShyrnal;    // Вызов процедуры заполнения списка тем
    getDataControlForShyrnal; // Вызов процедуры заполнения списка контрлей знаний
end;
